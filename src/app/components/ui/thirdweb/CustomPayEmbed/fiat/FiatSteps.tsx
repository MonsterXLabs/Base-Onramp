...
import { defineChain, NATIVE_TOKEN_ADDRESS, ThirdwebClient } from 'thirdweb';
...
import { TokenInfo, useChainMetadata } from 'thirdweb/react';
import { Spacer } from '../../Spacer';
import { Spinner } from '../../Spinner';
import { Container, Line, ModalHeader } from '../../basic';
import { Button, ButtonLink } from '../../buttons';
import { Text } from '../../text';
import { ERC20OrNativeToken, NATIVE_TOKEN } from '../util';
...
} from '../pay-transactions/statusMeta';
import { ChainMetadata, getChainMetadata } from 'thirdweb/chains';
...
export function FiatSteps(props: {
  ...
  const { data: toChain } = useChainMetadata(defineChain(toTokenMeta.chainId));
  ...
  useEffect(() => {
    const fetchChainMetadata = async () => {
      if (props.status?.status !== 'NOT_FOUND' && props.status?.destination) {
        const chain = await getChainMetadata(
          defineChain(props.status.destination.token.chainId),
        );
  ...
  const onRampName = onRampChain?.name || '';
  const onRampExplorers = onRampChain?.explorers || [];
  const toChainName = toChain?.name || '';
  const toChainExplorers = toChain?.explorers || [];
  const destinationName = destinationChain?.name || '';
  ...
  const onRampChainInfo = <Text size="xs">{onRampName}</Text>;
  ...
  return (
    <Container p="lg">
      ...
      <PaymentStep
        ...
        state={getStep1State()}
        explorer={
          onRampExplorers[0]?.url && onRampTxHash
            ? {
                label: 'View on Explorer',
                url: `${onRampExplorers[0]?.url}/tx/${onRampTxHash}`,
              }
            : undefined
        }
      />
      ...
      <PaymentStep
        ...
        state={getStep2State()}
        explorer={
          toChainExplorers[0]?.url && toTokenTxHash
            ? {
                label: 'View on Explorer',
                url: `${toChainExplorers[0].url}/tx/${toTokenTxHash}`,
              }
            : undefined
        }
      />

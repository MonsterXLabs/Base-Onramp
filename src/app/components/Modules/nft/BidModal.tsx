...
export default function BidModal({
  ...
  const [debouncedBidPrice] = useDebounce([value, day, hour], 500);
  useEffect(() => {
  ...
  const bidAsset = async () => {
    try {
      ...
      const duration = getSeconds(day, hour);
    ...
    return (
      <div className="flex flex-col gap-y-4 w-full">
        <div className="flex gap-x-3 items-center">
          <img src="/icons/info.svg" className="w-12" alt="info" />
  ...
  if (step == 5)
    return (
      <div className="flex flex-col gap-y-4 items-center text-center">
        <img src="/icons/refresh.svg" className="w-20 mx-auto" alt="refresh" />

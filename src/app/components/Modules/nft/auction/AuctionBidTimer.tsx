...
const calculateTimeRemaining = (
  ...
  const now = moment();
  const timeDifference = moment.duration(now.diff(bidStartTime));
...
const AuctionBidTimer = ({ bidTime }: { bidTime: string }) => {
  const [timeRemaining, setTimeRemaining] = useState<{
    secondsMode: boolean;
    result: string;
  }>(calculateTimeRemaining(bidTime));

  useEffect(() => {
    const updateTimer = () => {
      setTimeRemaining(calculateTimeRemaining(bidTime));
